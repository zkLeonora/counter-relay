...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const activeRole: UserRole = (user as any)?.role || userRole;
  ...
  const rawMainNavItems: { id: ViewType; labelKey: Parameters<typeof t>[0]; icon: React.ComponentType<{ className?: string }>; badge?: number }[] = [
  ...
  const mainNavItems = rawMainNavItems.filter((item) => canAccessView(activeRole, item.id));

  const rawCrmNavItems: { id: ViewType; labelKey: Parameters<typeof t>[0]; icon: React.ComponentType<{ className?: string }> }[] = [
    { id: 'customers', labelKey: 'customers', icon: Users },
    { id: 'reports', labelKey: 'reports', icon: BarChart3 },
  ];
  const crmNavItems = rawCrmNavItems.filter((item) => canAccessView(activeRole, item.id));

  const rawSystemNavItems: { id: ViewType; labelKey: Parameters<typeof t>[0]; icon: React.ComponentType<{ className?: string }> }[] = [
    { id: 'store', labelKey: 'store', icon: Store },
    { id: 'users', labelKey: 'users', icon: UserCheck },
    { id: 'settings', labelKey: 'settings', icon: Settings },
  ];
  const systemNavItems = rawSystemNavItems.filter((item) => canAccessView(activeRole, item.id));
